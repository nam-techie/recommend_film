...
export default function Navbar({ genres, countries }: { genres: Genre[]; countries: Country[] }) {
  ...
  return (
    <header className={`sticky top-0 z-50 w-full border-b border-white/[0.07] ${isHome ? 'bg-[#080911]/85' : 'bg-[#080911]/95'} backdrop-blur-xl`}>
      <div className="relative mx-auto flex h-16 max-w-shell items-center gap-3 px-3 sm:px-5 lg:h-[72px] lg:px-7">
        <button type="button" className="touch-target -ml-2 flex items-center justify-center text-fg-secondary lg:hidden" aria-label={mobileMenuOpen ? 'Đóng menu' : 'Mở menu'} aria-expanded={mobileMenuOpen} onClick={() => setMobileMenuOpen((value) => !value)}>{mobileMenuOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}</button>
...
function NavLink({ href, children }: { href: string; children: React.ReactNode }) {
  return <Link href={href} className="flex min-h-10 items-center rounded-xl px-3 text-sm font-semibold text-fg-secondary hover:bg-white/[0.06] hover:text-fg">{children}</Link>
}
...
function MenuDropdown({ label, open, onToggle, items, base, allHref, align = 'left', columns }: { label: string; open: boolean; onToggle: () => void; items: Array<Genre | Country>; base: string; allHref: string; align?: 'left' | 'right'; columns: string }) {
  return <div className="relative"><button type="button" onClick={onToggle} className={`flex min-h-10 items-center gap-1.5 rounded-xl px-3 text-sm font-semibold ${open ? 'bg-white/[0.08] text-fg' : 'text-fg-secondary hover:bg-white/[0.06]'}`} aria-haspopup="menu" aria-expanded={open}>{label}<ChevronDown className={`h-3.5 w-3.5 transition-transform ${open ? 'rotate-180' : ''}`} /></button>{open && <div role="menu" className={`absolute top-[calc(100%+10px)] z-[70] w-[620px] overflow-hidden rounded-2xl border border-white/10 bg-[#10131f]/[.98] shadow-[0_24px_70px_rgba(0,0,0,.55)] backdrop-blur-xl ${align === 'right' ? 'right-0' : 'left-0'}`}><div className="flex items-center justify-between border-b border-white/[0.07] px-4 py-3"><span className="text-sm font-bold text-fg">Khám phá theo {label.toLowerCase()}</span><Link href={allHref} className="text-xs font-semibold text-accent-soft hover:text-accent-soft">Xem tất cả</Link></div><div className={`grid ${columns} max-h-[360px] gap-1 overflow-y-auto p-3`}>{items.map((item) => <Link key={item.slug} href={`${base}/${item.slug}`} role="menuitem" className="rounded-xl px-3 py-2.5 text-sm text-fg-secondary hover:bg-accent/10 hover:text-accent-soft">{item.name}</Link>)}</div></div>}</div>
}

function MobileLink({ href, icon: Icon, children }: { href: string; icon: typeof Film; children: React.ReactNode }) {
  return <Link href={href} className="flex min-h-12 items-center gap-2 rounded-xl border border-white/[0.06] bg-white/[0.025] px-3 text-sm font-medium text-fg-secondary hover:bg-white/[0.06]"><Icon className="h-4 w-4 text-accent-soft" />{children}</Link>
}
