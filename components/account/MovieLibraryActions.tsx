...
export function MovieLibraryActions({ movie, compact = false }: { movie: { slug: string; title: string; poster?: string; year?: number }; compact?: boolean }) {
  ...
  return <div className="flex flex-wrap gap-2">
    ...
    <Button size={compact ? 'sm' : 'default'} variant={item?.favorite ? 'default' : 'outline'} disabled={account.degraded} onClick={() => void account.updateMovieLibrary(input, { favorite: !item?.favorite })} className={item?.favorite ? 'bg-bad hover:bg-bad' : ''}><Heart className={cn('h-4 w-4', item?.favorite && 'fill-current')} />Yêu thích</Button>
