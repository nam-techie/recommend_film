...
export function AccountActivityPanel({ sessions, activities }: { sessions: AccountSession[]; activities: SocialActivity[] }) {
  ...
  return <div className="space-y-6">
    ...
    <section>
      <div className="mb-3 flex items-end justify-between gap-3"><div><h2 className="text-lg font-semibold">Lịch sử online</h2><p className="mt-1 text-xs text-fg-muted">Tối đa 30 phiên gần nhất, gồm thời gian bắt đầu và kết thúc.</p></div></div>
      {ordered.length ? <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.025]">{ordered.slice(0, 12).map((session, index) => {
        ...
        return <div key={session.id} className="grid gap-3 border-b border-white/10 p-4 last:border-0 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="flex min-w-0 gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-accent/10 text-accent-soft"><Laptop className="h-5 w-5" /></span><div className="min-w-0"><p className="flex flex-wrap items-center gap-2 text-sm font-semibold text-fg">{session.device === 'mobile' ? 'Điện thoại' : session.device === 'tablet' ? 'Máy tính bảng' : 'Máy tính'} · {session.browser}{!session.endedAt && <span className="rounded-full bg-ok/15 px-2 py-0.5 text-xs text-ok">ONLINE</span>}</p><p className="mt-1 text-xs text-fg-muted">{session.timezone} · {session.locale}</p></div></div><div className="sm:text-right"><p className="text-xs text-fg-secondary">{formatMoment(session.startedAt)} → {session.endedAt ? formatMoment(session.endedAt) : 'hiện tại'}</p><p className="mt-1 text-xs text-fg-muted">Online {formatDuration(end - session.startedAt)}{offlineGap ? ` · trước đó offline ${formatDuration(offlineGap)}` : ''}</p></div></div>
      })}</div> : <div className="rounded-2xl border border-dashed border-white/10 py-12 text-center"><Clock3 className="mx-auto h-8 w-8 text-fg-muted" /><p className="mt-3 text-sm text-fg-muted">Phiên đầu tiên sẽ xuất hiện sau khi Database Rules mới được áp dụng.</p></div>}
    </section>
  </div>
}

function Stat({ icon: Icon, label, value, accent = false }: { icon: typeof Activity; label: string; value: string; accent?: boolean }) {
  return <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4"><Icon className={`mb-3 h-5 w-5 ${accent ? 'text-ok' : 'text-accent-soft'}`} /><p className="truncate text-lg font-bold text-fg">{value}</p><p className="mt-1 text-xs text-fg-muted">{label}</p></div>
}
