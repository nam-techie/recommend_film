...
export default function AccountControls({ user, logout }: { user: User; logout: () => Promise<void> }) {
  ...
  return (
    <div ref={menuRef} className="relative">
      <Button variant="outline" size="sm" aria-haspopup="menu" aria-expanded={open} onClick={() => setOpen((value) => !value)} className="h-10 gap-2 rounded-full border-white/10 bg-white/[0.035] px-2 text-fg hover:bg-white/[0.07] sm:px-3">
        <AccountAvatar name={name} src={account.profile?.avatar || user.photoURL} className="h-7 w-7 text-xs" />
        <span className="hidden max-w-24 truncate 2xl:inline">{name}</span>
        {account.unreadCount > 0 && <span className="absolute -right-0.5 -top-0.5 flex min-h-5 min-w-5 items-center justify-center rounded-full bg-accent px-1 text-xs font-bold text-fg">{Math.min(99, account.unreadCount)}</span>}
      </Button>
      {open && (
