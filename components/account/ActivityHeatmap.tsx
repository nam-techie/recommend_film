...
export function ActivityHeatmap({ activities, weeks = 18 }: { activities: SocialActivity[]; weeks?: number }) {
  ...
  return <section className="rounded-2xl border border-white/10 bg-[#0b0e18] p-4 sm:p-5">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-2"><div><h3 className="font-semibold text-fg">Dấu ấn xem phim</h3><p className="mt-1 text-xs text-fg-muted">{total} hoạt động trong {weeks} tuần gần nhất</p></div><div className="flex items-center gap-1 text-xs text-fg-muted"><span>Ít</span>{[0, 1, 2, 3, 4].map((level) => <span key={level} className={cn('h-3 w-3 rounded-sm', level === 0 ? 'bg-white/[0.055]' : level === 1 ? 'bg-accent/25' : level === 2 ? 'bg-accent/50' : level === 3 ? 'bg-accent/75' : 'bg-accent')} />)}<span>Nhiều</span></div></div>
    <div className="overflow-x-auto pb-1"><div className="grid w-max grid-flow-col grid-rows-7 gap-1" role="img" aria-label={`Biểu đồ ${total} hoạt động trong ${weeks} tuần`}>
      {days.map(({ date, count }) => {
        ...
        return <span key={date.toISOString()} title={count < 0 ? '' : `${label}: ${count} hoạt động`} className={cn('h-3.5 w-3.5 rounded-sm ring-1 ring-inset ring-white/[0.035]', count < 0 ? 'bg-transparent ring-0' : level === 0 ? 'bg-white/[0.055]' : level === 1 ? 'bg-accent/25' : level === 2 ? 'bg-accent/50' : level === 3 ? 'bg-accent/75' : 'bg-accent')} />
      })}
    </div></div>
  </section>
}
