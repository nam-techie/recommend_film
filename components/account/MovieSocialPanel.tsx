...
export function MovieSocialPanel({ movie }: { movie: { slug: string; title: string; poster?: string; year?: number } }) {
  ...
  return <section className="mt-10 space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-3">
      <div><p className="text-sm text-accent-soft">Cộng đồng</p><h2 className="mt-1 text-2xl font-bold text-fg">Danh sách và đánh giá</h2><p className="mt-1 text-sm text-fg-secondary">Lưu phim hoặc chia sẻ cảm nhận với người xem khác.</p></div>
      {user ? <div className="flex flex-wrap gap-2"><Button variant={libraryItem?.watchLater ? 'default' : 'outline'} disabled={account.degraded} onClick={() => void account.updateMovieLibrary(libraryMovie, { watchLater: !libraryItem?.watchLater })}><Bookmark className="h-4 w-4" />{libraryItem?.watchLater ? 'Đã thêm Xem sau' : 'Xem sau'}</Button><Button variant={libraryItem?.favorite ? 'default' : 'outline'} disabled={account.degraded} onClick={() => void account.updateMovieLibrary(libraryMovie, { favorite: !libraryItem?.favorite })} className={libraryItem?.favorite ? 'bg-bad hover:bg-bad' : ''}><Heart className={cn('h-4 w-4', libraryItem?.favorite && 'fill-current')} />Yêu thích</Button><Button variant={libraryItem?.watchStatus === 'completed' ? 'default' : 'outline'} disabled={account.degraded} onClick={() => void account.updateMovieLibrary(libraryMovie, { watchStatus: libraryItem?.watchStatus === 'completed' ? null : 'completed' })}><Check className="h-4 w-4" />{libraryItem?.watchStatus === 'completed' ? 'Đã xem' : 'Đánh dấu đã xem'}</Button></div> : <AuthDialog><Button variant="outline"><Bookmark className="mr-2 h-4 w-4" />Lưu phim</Button></AuthDialog>}
    </div>

    {user && account.profile && !account.degraded ? <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 sm:p-5">
      <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-semibold text-fg">{reviews.some((item) => item.authorUid === user.uid) ? 'Chỉnh sửa đánh giá' : 'Viết đánh giá'}</h3><div className="flex items-center gap-1" aria-label={`Điểm ${rating} trên 10`}>{Array.from({ length: 10 }, (_, index) => index + 1).map((value) => <button key={value} type="button" aria-label={`${value} điểm`} onClick={() => setRating(value)} className={cn('flex h-8 w-8 items-center justify-center rounded-lg text-xs font-semibold transition', value <= rating ? 'bg-rating/15 text-rating' : 'bg-white/5 text-fg-muted hover:text-fg')}>{value}</button>)}</div></div>
      <textarea value={content} onChange={(event) => { setContent(event.target.value.slice(0, 1200)); setNotice(null) }} rows={4} placeholder="Điều gì khiến bộ phim đáng xem?" className="mt-4 w-full rounded-xl border border-white/10 bg-black/25 px-3 py-3 text-sm text-fg outline-none focus:ring-2 focus:ring-accent" />
      <div className="mt-3 flex flex-wrap items-center gap-3"><label className="flex cursor-pointer items-center gap-2 text-xs text-fg-secondary"><input type="checkbox" checked={spoiler} onChange={(event) => setSpoiler(event.target.checked)} className="accent-accent" />Có nội dung tiết lộ</label><span className={cn('text-xs', trimmedLength > 0 && trimmedLength < MIN_REVIEW_LENGTH ? 'text-rating' : 'text-fg-muted')}>{content.length}/1200</span>{trimmedLength > 0 && trimmedLength < MIN_REVIEW_LENGTH && <span className="text-xs text-rating">Cần ít nhất {MIN_REVIEW_LENGTH} ký tự.</span>}{notice && <span role="status" className={cn('text-xs', noticeError ? 'text-bad' : 'text-ok')}>{notice}</span>}<Button size="sm" disabled={saving || trimmedLength < MIN_REVIEW_LENGTH || rating < 1} onClick={() => void save()} className="ml-auto">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <><MessageSquareText className="mr-2 h-4 w-4" />Lưu đánh giá</>}</Button></div>
    </div> : <div className="rounded-2xl border border-dashed border-white/10 p-8 text-center text-sm text-fg-secondary">{user ? 'Dữ liệu tài khoản chưa kết nối. Hãy thử lại trong trang Tài khoản.' : 'Đăng nhập để viết đánh giá và tương tác.'}</div>}

    <div className="space-y-3">{reviews.length ? reviews.map((review) => <SocialReviewCard key={review.id} review={review} actor={account.degraded ? null : account.profile} onReviewDeleted={(deleted) => { if (deleted.authorUid === user?.uid) { setContent(''); setRating(8); setSpoiler(false); setNotice('Đã xóa đánh giá.'); setNoticeError(false) } }} />) : <div className="rounded-2xl border border-dashed border-white/10 p-10 text-center"><Star className="mx-auto h-7 w-7 text-fg-muted" /><p className="mt-3 text-sm text-fg-secondary">Chưa có đánh giá. Hãy là người đầu tiên.</p></div>}</div>
  </section>
}
