...
export function CatalogPage({ title, description, movies, query, totalItems = movies.length, totalPages = 1, genres = [], countries = [], showSearch = false }: CatalogPageProps) {
  ...
  return <div className="mx-auto min-h-[70vh] max-w-shell px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
    {showSearch ? (
    ...
    {movies.length ? <MovieGrid movies={movies} /> : <div className="rounded-3xl border border-dashed border-white/10 bg-white/[0.02] py-24 text-center"><Clapperboard className="mx-auto h-9 w-9 text-fg-muted" /><p className="mt-4 font-semibold text-fg-secondary">Không tìm thấy phim phù hợp</p><p className="mt-1 text-sm text-fg-muted">Hãy thử từ khóa ngắn hơn hoặc đặt lại bộ lọc.</p></div>}

    {totalPages > 1 && <nav className="mt-12 flex items-center justify-center gap-3" aria-label="Phân trang">{previous ? <Link href={previous} aria-label="Trang trước" className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/[0.045] text-fg hover:border-accent/30 hover:bg-accent/10"><ChevronLeft className="h-5 w-5" /></Link> : <span className="flex h-11 w-11 items-center justify-center rounded-full bg-white/[0.02] text-fg-muted"><ChevronLeft className="h-5 w-5" /></span>}<span className="flex h-11 items-center rounded-full border border-white/10 bg-white/[0.045] px-5 text-sm font-semibold text-fg">Trang {query.page} <span className="mx-1.5 text-fg-muted">/</span> {totalPages}</span>{next ? <Link href={next} aria-label="Trang sau" className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/[0.045] text-fg hover:border-accent/30 hover:bg-accent/10"><ChevronRight className="h-5 w-5" /></Link> : <span className="flex h-11 w-11 items-center justify-center rounded-full bg-white/[0.02] text-fg-muted"><ChevronRight className="h-5 w-5" /></span>}</nav>}
  </div>
}
