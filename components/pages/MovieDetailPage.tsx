          ...
          {tab === 'episodes' && (
            <div className="p-5 sm:p-7">
              <div className="flex flex-col gap-4 border-b border-white/[0.07] pb-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap gap-2">{episodes.map((server, index) => <button key={`${server.server_name}-${index}`} type="button" onClick={() => { setSelectedServer(index); setSelectedEpisode(0) }} className={`min-h-10 rounded-xl border px-4 text-sm font-bold transition-colors ${selectedServer === index ? 'border-accent/60 bg-gradient-to-r from-accent/25 to-accent-strong/20 text-accent-soft shadow-[0_0_0_3px_rgba(217,70,239,.06)]' : 'border-white/10 bg-black/15 text-fg-secondary hover:border-white/20 hover:text-fg'}`}>{server.server_name}<span className="ml-2 text-xs opacity-60">{server.server_data.length} tập</span></button>)}</div>
              </div>
              <div className="mt-6 grid grid-cols-2 gap-2.5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8">{episodes[selectedServer]?.server_data.map((episode, index) => <button key={`${episode.slug}-${index}`} type="button" onClick={() => selectEpisode(selectedServer, index, showPlayer)} className={`group flex min-h-12 items-center justify-center gap-2 rounded-xl border px-3 text-sm font-bold transition-[color,background-color,border-color] ${selectedEpisode === index ? 'border-accent/60 bg-gradient-to-br from-accent/25 to-accent-strong/20 text-fg' : 'border-white/[0.09] bg-black/20 text-fg-secondary hover:border-accent/30 hover:bg-accent/[0.07] hover:text-fg'}`}><PlayCircle className={`h-4 w-4 ${selectedEpisode === index ? 'text-accent-soft' : 'text-fg-muted group-hover:text-accent-soft'}`} />{episode.name}</button>)}</div>
            </div>
          )}
