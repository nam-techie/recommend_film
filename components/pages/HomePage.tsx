        ...
        {data.genres.length > 0 && (
          <section className="deferred-section rounded-3xl border border-white/[0.07] bg-white/[0.025] p-5 sm:p-7">
            <div className="mb-5 flex items-end justify-between gap-4">
              <div>
                <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-accent-soft">
                  <Sparkles className="h-4 w-4" /> Khám phá theo sở thích
                </div>
                <h2 className="text-2xl font-bold text-fg sm:text-3xl">Bạn đang quan tâm gì?</h2>
              </div>
              <Link href="/genres" className="hidden items-center gap-1 text-sm font-semibold text-fg-secondary hover:text-fg sm:flex">
              ...
              {data.genres.slice(0, 12).map((genre, index) => (
                <Link
                  ...
                  className={`rounded-full border px-4 py-2.5 text-sm font-medium transition-colors ${
                    index < 4
                      ? 'border-accent/30 bg-accent/10 text-accent-soft hover:bg-accent/20'
                      : 'border-white/10 bg-white/[0.035] text-fg-secondary hover:border-white/25 hover:text-fg'
                  }`}
                >
                  {genre.name}
                </Link>
