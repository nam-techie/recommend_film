...
export function CreateWatchPartyDialog({ children, movieSlug, movieTitle, movieOriginalTitle, moviePoster, movieYear, movieDuration, movieType, movieGenres, movieQuality, movieLanguage, movieRating, movieVideoUrl, episodes = [], initialEpisodeId }: Props) {
  ...
  const canCreateRoom = capabilities.canCreateRoom
  ...
  useEffect(() => {
    if (capabilities.roomAccessModes.length && !capabilities.roomAccessModes.includes(accessMode)) setAccessMode('link_only')
  }, [accessMode, capabilities.roomAccessModes])
  ...
  const create = async (replaceActiveRoom = false) => {
    ...
    if (!canCreateRoom) { setError('CinePass Plus hoặc CinePass Ultra mới có thể tạo phòng xem chung.'); return }
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild>{children}</DialogTrigger><DialogContent className="border-white/10 bg-[#111522] sm:max-w-lg">
    ...
    <div className="space-y-5">
      ...
      {user && !entitlementLoading && !canCreateRoom && <div className="rounded-xl border border-warn/25 bg-warn/10 p-4"><p className="text-sm font-semibold text-warn">{PLAN_DEFINITIONS[accountPlan].name} chỉ có thể tham gia phòng ở chế độ xem.</p><p className="mt-1 text-xs leading-5 text-fg-secondary">Nâng lên CinePass Plus để tạo phòng bằng link, chat và reaction.</p><Button asChild size="sm" className="mt-3"><Link href="/pricing">Xem các gói nâng cấp</Link></Button></div>}
      ...
      <Button className="w-full" disabled={!selected || isCreating || authLoading || entitlementLoading || !user || !canCreateRoom || Boolean(activeRoom) || (accessMode === 'password' && password.length < 6)} onClick={() => void create()}>{isCreating ? <><Loader className="mr-2 h-4 w-4 animate-spin" />Đang tạo phòng…</> : canCreateRoom ? `Tạo phòng · tối đa ${capabilities.roomMaxMembers} người` : 'Cần CinePass Plus'}</Button>
    </div>
